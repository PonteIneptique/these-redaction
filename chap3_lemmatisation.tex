...
\begin{enumerate}
    ...
    \item 1.16\% sont des verbes dont le participe présent rentre en collision avec le lemme attendu
    \item 0.96\% sont des erreurs inversées de 3.
    \item 3.59\% sont des noms partageant leur nominatif avec une forme adjectivale
...
Dans un cadre plus large, sur 4~398 erreurs, 2~942 portent sur des tokens ambigüs, connaissant donc plusieurs solutions de lemmatisation, soit 8,08\% de ceux-ci. 451 erreurs portent sur des lemmes inconnus en phase d'entraînement, soit 40\% de ceux-ci.
...
\ref{subsec:lemmatisation:hors-domaine}
...
Dans cette étude, deux prosateurs font face à la prose de Sénèque sous deux formes (ses dialogues et traités d'une part, ses lettres d'autre part) et aux vers de Sénèque et de Plaute. On note tout de suite une nette relation entre accuracy et forme, puisque à la fois Catulle et Sénèque ont des R et des \textit{accuracies} particulièrement hauts sur Catulle et le théâtre de Sénèque. Au contraire, l'oeuvre versifié de Sénèque ne semble pas mieux comprendre Sńèque que d'autres prosateurs (résultats similaires entre Ciçeron, \textit{De Amicitia}, \textit{In Catilinam} et le \textit{De Brevitate Vitae} de Sénèque). L'inverse est aussi remaerquée puisqu'aucun des modèles de prose de Sénèque ne réagit nettement mieux que celui de Tacite, en accuracy ou en résidu de Pearson. Les auteurs de proses fonctionnent bien sur eux-mêmes et Cicéron ainsi que Tacite fonctionnent particulièrement bien sur Salluste: on peut supposer que les \textit{Verrines} et les \textit{Philippiques} de Cicéron, formant le corpus d'entraînement ici, partagent avec Tacite et les \textit{Catilinaires} de Salluste des questions de pouvoir politique, de corruption et - par extension - un vocabulaire et un style assez proche. Au contraire, les oeuvres philosophiques de Sénèque ne sont pas assez marquées, et en tout cas top éloignées du \textit{De Amicitia} dans leur style pour influer de quelque manière. Pour finir, après deux tests, on se rend compte du caractère presque neutre du Satyricon de Pétrone qui ne semble profiter ni de la prose ni du vers ni encore de la période, mais qui profite tout de même du passage de 90~000 à 115~000 tokens du corpus d'entraînement
...
\begin{itemize}
    ...
    \item Le genre, comme la philosophie ou l'histoire en figure \ref{fig:lemmatisation:longree:auteurVSforme}, ont un relativement faible impact. Il se pose la question de savoir s'il s'agit avant toute chose d'un partage de vocabulaire liés à des événements rapportés et communs ou bien un véritable apprentissage de genre.
\end{itemize}
