...
\begin{quote}[Horace, Satyres, 1.5.100]{Porphyrion}
    ...
    \enquote{Il assigne un nom extrêmement spirituel au Juif en l'appelant ‘Apella’, sous prétexte que les juifs n'ont pas de peau sur les parties génitales. \footnote{Porphyrion fait ici un rapprochement entre une préfixation privative \textit{a-} et le mot ``pellis'' (peau) et le nom hébreux -- semble-t-il commun -- d'Apellas} ( \url{https://poj.peeters-leuven.be/secure/POJ/downloadpdf.php?ticket_id=60e6b9ae27f24} )}
\end{quote}

Ces deux premiers exemples, bien que légèrement différents, ne laissent pas beaucoup de doute sur le sens des deux mots sélectionnés. 

Du côté de Suétone, \textit{libido}, proche de \textit{iuvenes senesque}, laisse peu de doute, y compris hors contexte, au lecteur humain. Le modèle retient bien l'attention 100\% du temps sur le terme relevé par J. N. Adams mais décide de ne pas le classer positif, probablement à cause de sa polysémie. On peut faire l'hypothèse que la présence de forum, bien que présent 6 fois dans le corpus d'Adams, puisse pousser vers une lecture autre de \textit{libido}. % Petite étude de proximité libido ~ iuvenes ?

Du pointe de vue de Porphyrion, commentateur d'Horace, le résultat est plus difficile à comprendre. Dix-neuf fois sur vingt, le modèle a retenu l'importance de \textit{genitali}, cooccurrent de \textit{parte} par ailleurs, mais estime que la phrase ne parle pas de sexualité. Si de fait, il n'est pas question d'acte, on a bien ici une description d'une partie de l'anatomie masculine avec une mention du rite de circoncision. La classification négative tient peu, % CHECK LES STATS COUILLON !
...
Plautus {[...]} sometimes uses [the verb subigito] in a sexual sense, was a much milder. {[...]} Subigito in Plautus suggests no more than 'make advances to, lay hands on'. 
p. 323 Prostitutes Adams. Le doute est ici permis dans le texte (moeurs marseillaises (homophiles) au vers précédent, mention de la chambre au vers suivant et à la scène qui vient de s'y passer.
...
% Pétrone, 0 Siècle ()
% Pétrone, 0 Siècle (urn:cts:latinLit:phi0972.phi001.perseus-lat2)
%  
...
Le poème est sexuel, mais très difficile d'en tirer l'information sans le contexte ici. Même dans le poème, le lien est léger.
Adams le compare à des passages BEAUCOUP plus clairs en p. 159 (cum te complexa morientem, Galle, puella uidimus (Prop. 1.10.5)

% Ovide, -1 Siècle (urn:cts:latinLit:phi0959.phi001.perseus-lat2)
...
Ici facio = futuo ou pedico (204 Adams). En dehors de l'indice d'interdit, il n'y a rien qui ne pousse nécessairement vers le sexe, sans contexte. C'est le reste des amours (au vers précédent, répétition de \textit{casta est} (elle est chaste), au vers suivant \textit{adultera}). Mais sans ce contexte, le vers est difficilement classable, même pour le lecteur humain.
...
\begin{quote}[Épigrammes, 106]{Ausone}
...
Deux problèmes se posent pour la détection: il s'agit d'un extrait très court, la phrase n'est pas complète. En recopiant ``bêtement'' l'extrait chez Adams (cité fois (114, 172, 240) et rapproché du texte de Martial (11.99.5), plus claire (sic constringuntur magni Symplegade culi). Même avec le contexte\footnote{\enquote{Il tourne alors ses bras, comme sous le dieu la Ménade : poitrine, jambes, flanc, épigastre, aines, mollets, cuisses dos, épaule, encolure, antre bouseux des symplégades, aux lieux les plus divers se perd le bras vagabond du bourreau, jusqu’ à ce que, par la vapeur engourdi du bain chaud, son prurit caressant s’ apaise en mortelles langueurs.}, \cite{ausone_d_2010}}
