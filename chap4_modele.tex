...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/chap4/rangs_distrib.png}
...
Dans un premier temps, on s'intéresse à la capacité de l'attention à reconnaître dans chacune des documents échantillons l'importance du mot retenu par J. N. Adams dans son lexique. Pour faire cela, on analyse la fréquence des rang de ces derniers. Cette méthodologie pose des limites: dans certaines phrases, la ``charge'' peut-être multiple, et J. N. Adams a pu relever le même échantillon plusieurs fois mais pour plusieurs mots différents. Ainsi, dans la phrase ``\textit{cum cunno mihi mentula est uocanda. }'' (``Il faut appeler cela une bitte et cela une chatte.''), les mots \textit{mentula} (bitte) et \textit{cunnus} (chatte) portent tous les deux une importante forte. D'après l'analyse des termes et de leur rang en figure \ref{fig:chap4:ranks-attention}, on peut conclure que le modèle intègre largement les bons mots porteurs, avec quelques rares situations où il passe vraisemblablement à côté. Le rang médian des mots de J. N. Adams est en effet 2 (donc deuxième mot portant le plus d'attention) avec un rang relatif aux environs de 6\% des mots les plus importants de la phrase.

% Nous proposons alors quelques études de cas de faux-positifs avec les mots retenus comme étant les plus importants par le modèle ainsi que de vrais négatifs avec des att

% Analyse de la position ANA / Rank Attention

% Analyse des faux positifs

%Dans la phrase ['ille', 'uxor', 'aut', 'amica', 'riualis1', 'lasciuio', 'languidus', ',', 'precor', ',', 'reddo', ','], l'attention sera probablement moins portée sur uxor / amica que sur lasciuio et languidus.
